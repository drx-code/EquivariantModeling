...
def get_kl(mu, logvar):
    ...
    return loss_prior
# def get_kl_beta(alpha, beta):
#     # Return KL divergence between Beta(alpha, beta) and Beta(1, 1), divided by data dimension.
#     # alpha_1 = torch.tensor([1.], device=alpha.device)
#     # beta_1 = torch.tensor([1.], device=beta.device)
#     alpha_1 = torch.ones_like(alpha)
#     beta_1 = torch.ones_like(beta)
#     eps = 1e-10
#     term1 = torch.lgamma( alpha+beta) + torch.lgamma( alpha_1) + torch.lgamma( beta_1) \
#         - torch.lgamma( alpha_1+beta_1) - torch.lgamma( alpha) -  torch.lgamma( beta)
#     term2 = (alpha - alpha_1)*(torch.polygamma(0,alpha) - torch.polygamma(0,alpha + beta))
#     term3 = (beta - beta_1)*(torch.polygamma(0,beta) - torch.polygamma(0,alpha + beta))
#     # print(term1.mean(), term2.mean(), term3.mean())
#     kl = term1 + term2 + term3
#     loss_prior = torch.mean(kl) /  (alpha.shape[1]*alpha.shape[2]*alpha.shape[3])
#     return loss_prior
def get_kl_beta(alpha, beta):
    alpha_1 = torch.ones_like(alpha)
    beta_1 = torch.ones_like(beta)
    kl = kl_divergence(torch.distributions.Beta(alpha, beta), torch.distributions.Beta(alpha_1, beta_1))
    loss_prior = torch.mean(kl) /  (alpha.shape[1]*alpha.shape[2]*alpha.shape[3])
    return loss_prior

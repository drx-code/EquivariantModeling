...
class MetricLogger(object):
    def __init__(self, delimiter="\t", log_file="logs/"):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        self.log_file = log_file + '/output.log'
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        if is_main_process():
            message = '{} Total time: {} ({:.4f} s / it)'.format(
...
class NativeScalerWithGradNormCount:
    ...
    def __init__(self, float32=False):
        self._scaler = torch.cuda.amp.GradScaler(enabled=False if float32 else True)
